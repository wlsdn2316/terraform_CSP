...
data "azurerm_resource_group" "rg-az01-co013601-azgov-infra-01" {
	provider = azurerm.A
...
data "azurerm_virtual_network" "vnet-az01-azgov-iac-01" {
  name                = "vnet-az01-azgov-iac-01"
  resource_group_name = data.azurerm_resource_group.rg-az01-co013601-azgov-infra-01.name
  provider            = azurerm.A
}

data "azurerm_subnet" "sbn-az01-azgov-cloudwiz-app" {
  provider             = azurerm.A
  name                 = "sbn-az01-azgov-cloudwiz-app"
  resource_group_name  = data.azurerm_resource_group.rg-az01-co013601-azgov-infra-01.name
  virtual_network_name = data.azurerm_virtual_network.vnet-az01-azgov-iac-01.name
}
...
#   location            = azurerm_resource_group.rg-az01-co013601-azgov-infra-01.location
#   resource_group_name = azurerm_resource_group.rg-az01-co013601-azgov-infra-01.name
#   route {
...
data "azurerm_route_table" "udr_app" {
  provider            = azurerm.A
  name                = "udr-az01-prd-test-app"
...
#   provider       = azurerm.A
#   subnet_id      = data.azurerm_subnet.sbn-az01-azgov-cloudwiz-app.id
